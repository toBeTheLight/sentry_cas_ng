...
..  code-block:: python
    ...
    import sentry_cas_ng.views

    url(r'^accounts/login$', sentry_cas_ng.views.login, name='cas_ng_login'),
    url(r'^accounts/logout$', sentry_cas_ng.views.logout, name='cas_ng_logout'),
...
..  code-block:: python
    ...
    url(r'^accounts/callback$', sentry_cas_ng.views.callback, name='cas_ng_proxy_callback'),
...
..  code-block:: python
    ...
    from sentry_cas_ng import views as baseviews
...
..  code-block:: python
    ...
    from sentry_cas_ng.backends import CASBackend
...
backend that inherits from ``sentry_cas_ng.backends.CASBackend`` and override
...
sentry_cas_ng.signals.cas_user_authenticated
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
sentry_cas_ng.signals.cas_user_logout
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Allow on the CAS config sentry_cas_ng to act as a Proxy application.
Then after a user has logged in using the CAS, you can retrieve a Proxy Ticket as follow:

..  code-block:: python

    from sentry_cas_ng.models import ProxyGrantingTicket
...
inside of the sentry_cas_ng directory. Where ``lang_code`` is the language code for which you want to submit a
translation. Then open the file ``sentry_cas_ng/locale/lang_code/LC_MESSAGES/django.po`` with a gettex translations
editor (for example https://poedit.net/). Translate and save the file.
Think to add ``sentry_cas_ng/locale/lang_code/LC_MESSAGES/django.po`` to repo. Please do not add ``sentry_cas_ng/locale/lang_code/LC_MESSAGES/django.mo`` to repo since .mo file can be generated by .po file.
